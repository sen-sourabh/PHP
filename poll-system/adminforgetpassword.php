...
<div id="forget">
	<div class="container">
		<div cass="row">
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xm-12">
				<center>
					<h3 style="font-family: candara;"><b>ADMIN FORGET PASSWORD</b></h3>
						<form action="" name="forgetform" ng-app="" method="post">
							<input class="form-control" type="email" name="email" placeholder="Email" required="required" ng-model="adminemail"/>
							<span ng-show="forgetform.email.$error.email" style="color:red;font-weight: bold;">* Not a valid e-mail address</span><br>
...
if(isset($_POST['send']))
{
	...
	if(empty($useremail))
	{
		...
		echo "<script>window.location='http://localhost/phpdoc/project/adminforgetpassword.php'</script>";
	}
	else
	{
		$query = mysqli_query($connection,"SELECT email FROM admintable WHERE email LIKE '$useremail'");
		$row = mysqli_fetch_array($query);
		$num = mysqli_num_rows($query);
		if($num >= 1)
		{
			$to=$row['email']; // Receiver Email ID, Replace with your email ID
